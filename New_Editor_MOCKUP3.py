...
from PyQt5.QtWidgets import (
    ...
    QButtonGroup, QToolButton, QFrame, QSizePolicy, QGridLayout, QMenu
)
...
class SheetLabel(QWidget):
    ...
    def paintEvent(self, event):
        ...
        qp.setPen(QPen(QColor("#2c7ee9") if self.selected else QColor("#333"), 4*s if self.selected else 2*s))
...
class SelectionManager:
    def __init__(self, labels):
        self.labels = labels  # list of SheetLabel
        self.selected = []
...
class LabelSheetEditor(QWidget):
    def __init__(self, font_list):
        ...
        self.labels = [blank_label() for _ in range(9)]  # 3x3 grid
        self.clipboard = None
        ...
        self.align_left = QToolButton(); self.align_left.setText("L"); self.align_left.setCheckable(True)
        self.align_center = QToolButton(); self.align_center.setText("C"); self.align_center.setCheckable(True)
        self.align_right = QToolButton(); self.align_right.setText("R"); self.align_right.setCheckable(True)
        ...
        # Color palettes
        pal_h = QHBoxLayout()
        pal_h.addWidget(QLabel("Текст:"))
        self.font_color_btns = []
        for col in PALETTE_COLORS:
            b = QToolButton()
            b.setFixedSize(26, 26)
            b.setStyleSheet(f"background:{col}; border: 1px solid #888; border-radius: 4px;")
            b.clicked.connect(partial(self.set_field_color, "font_color", col))
            self.font_color_btns.append(b)
            pal_h.addWidget(b)
        pal_h.addSpacing(16)
        pal_h.addWidget(QLabel("Фон:"))
        self.bg_color_btns = []
        for col in PALETTE_COLORS:
            b = QToolButton()
            b.setFixedSize(26, 26)
            b.setStyleSheet(f"background:{col}; border: 1px solid #888; border-radius: 4px;")
            b.clicked.connect(partial(self.set_field_color, "bg_color", col))
            self.bg_color_btns.append(b)
            pal_h.addWidget(b)
        left_panel.addLayout(pal_h)
    ...
    def update_toolbar_from_field(self):
        ...
        for i, b in enumerate(self.font_color_btns):
            b.setStyleSheet(
                f"background:{PALETTE_COLORS[i]}; border: 3px solid {'#222' if color_same and f['font_color']==PALETTE_COLORS[i] else '#eee'}; min-width:24px;")
        for i, b in enumerate(self.bg_color_btns):
            b.setStyleSheet(
                f"background:{PALETTE_COLORS[i]}; border: 3px solid {'#222' if bg_same and f['bg_color']==PALETTE_COLORS[i] else '#eee'}; min-width:24px;")
