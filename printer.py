...
def _font_for_style(family: str, bold: bool, italic: bool) -> str:
    key = family.lower()
    if bold and italic: key += " bold italic"
    elif bold:          key += " bold"
    elif italic:        key += " italic"

    path = SYSTEM_FONTS.get(key) or SYSTEM_FONTS.get(family.lower())
    if not path or not path.exists():
        key = ("dejavu bold italic" if (bold and italic) else
               "dejavu bold" if bold else
               "dejavu italic" if italic else "dejavu")
        path = SYSTEM_FONTS[key]

    _register_font(key, path)
    return key

# ───────── draw one label (with line-split & auto-resize) ──────────────
def _draw_label(c: canvas.Canvas, lbl, x_pt: float, y_pt: float, app):
    ...
    if lbl.name_main.strip():
        mains, ms = fit_text(
            lbl.name_main, app.name_font_size.get(),
            max_w, app.name_font_family.get(),
            app.name_bold.get(), app.name_italic.get()
        )
        gap = BIGGER_GAP if not has_sub else NORMAL_GAP
        for line in mains:
            lines.append((line, ms,
    ...
    if has_sub:
        subs, ss = fit_text(
            lbl.name_sub, app.sub_font_size.get(),
            max_w, app.sub_font_family.get(),
            app.sub_bold.get(), app.sub_italic.get()
        )
        for line in subs:
            lines.append((line, ss,
    ...
    for r in range(TEMPLATE.rows):
        for col in range(TEMPLATE.cols):
            ...
            c.rect(mm_to_pt(x_mm),
                   ...
                   mm_to_pt(TEMPLATE.label_h_mm), stroke=1, fill=0)
    c.save()
    try:
        if platform.system() == "Windows":
            os.startfile(tmp.name, "print")
